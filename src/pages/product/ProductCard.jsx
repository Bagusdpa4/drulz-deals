...
export const ProductCard = ({ product, onAdd }) => {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-neutral-300 bg-slate-100/70 shadow-lg transition-shadow hover:shadow-xl">
      <div className="relative flex aspect-square h-40 w-full items-center justify-center bg-white p-3 sm:h-60">
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        <h3 className="text-md line-clamp-2 font-semibold leading-snug text-neutral-900">
          {product.name}
        </h3>
        {sizeKeys.length > 1 && (
          ...
          <button
            ...
            onClick={() =>
              onAdd?.({
                ...product,
                selectedSize,
                finalPrice: activePrice?.discPrice ?? activePrice?.price,
              })
            }
